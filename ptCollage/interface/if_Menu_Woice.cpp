...
#define _MENU_HEIGHT      80

#define _BUTTON_CHANGE_X   8
#define _BUTTON_CHANGE_Y   8
#define _BUTTON_EDIT_X     8
#define _BUTTON_EDIT_Y    24
#define _BUTTON_REMOVE_X   8
#define _BUTTON_REMOVE_Y  40
#define _BUTTON_EXPORT_X   8
#define _BUTTON_EXPORT_Y  56
...
{
	if( y + WOICENAME_HEIGHT + _MENU_HEIGHT < _rc_view.b - 8 ) _menu.bTop = false;
	else                                                       _menu.bTop = true;

	_menu.bActive = true;
	_menu.x       = x;
	_menu.u       = u;
	if( !_menu.bTop ) _menu.y = y + WOICENAME_HEIGHT;
...
void if_Menu_Woice_GetPosition( int32_t *px, int32_t *py ){ *px = _menu.x; *py = _menu.y; }
int32_t  if_Menu_Woice_GetWoiceIndex(){ return _menu.u;}
...
{
	if( !_menu.bActive ) return enum_WoiceMenu_None;

	if( x >= _menu.x + _BUTTON_CHANGE_X                  &&
		x <  _menu.x + _BUTTON_CHANGE_X + _BUTTON_WIDTH  &&
...
void if_Menu_Woice_Put()
{
	...
	fRECT rcBox = {256, 80,384, 96};
	float y;

	if( !_menu.bActive ) return;

	if( _menu.bTop ) y = _menu.y + _MENU_HEIGHT;
	else             y = _menu.y - WOICENAME_HEIGHT;

	g_dxdraw->tex_Put_View( _menu.x, _menu.y, &rc[0], SURF_MENUS );
	g_dxdraw->tex_Put_View( _menu.x,       y, &rcBox, SURF_MENUS );

	switch( _menu.menu ){
	case enum_WoiceMenu_Change: g_dxdraw->tex_Put_View( _menu.x + _BUTTON_CHANGE_X, _menu.y + _BUTTON_CHANGE_Y, &rcActive[ 0 ], SURF_MENUS ); break;
	case enum_WoiceMenu_Edit  : g_dxdraw->tex_Put_View( _menu.x + _BUTTON_EDIT_X  , _menu.y + _BUTTON_EDIT_Y  , &rcActive[ 1 ], SURF_MENUS ); break;
	case enum_WoiceMenu_Remove: g_dxdraw->tex_Put_View( _menu.x + _BUTTON_REMOVE_X, _menu.y + _BUTTON_REMOVE_Y, &rcActive[ 2 ], SURF_MENUS ); break;
	case enum_WoiceMenu_Export: g_dxdraw->tex_Put_View( _menu.x + _BUTTON_EXPORT_X, _menu.y + _BUTTON_EXPORT_Y, &rcActive[ 3 ], SURF_MENUS ); break;
	}
}
