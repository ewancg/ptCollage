...
static _MENUSTRUCT_ZOOM _zoom_menu = { 0 };
...
int32_t if_Menu_Zoom_GetIndex( float x, float y )
{
	...
	_zoom_menu.zoom_index = (int32_t)( y - _zoom_menu.y - 20 ) / _BUTTON_H;
...
{
	fRECT rc_menu = {400, 48,456,168};
	fRECT rc      = {456, 0, 512,  0};

	if( !_zoom_menu.b_active ) return;

	g_dxdraw->tex_Put_View( _zoom_menu.x, _zoom_menu.y, &rc_menu, SURF_PARTS );

	if( _zoom_menu.zoom_index != -1 )
	{
		rc.t = _zoom_menu.zoom_index * _BUTTON_H + rc_menu.t + 20;
		rc.b = rc.t +                  _BUTTON_H     ;
		g_dxdraw->tex_Put_View( _zoom_menu.x, _zoom_menu.y + 20 + _zoom_menu.zoom_index * _BUTTON_H, &rc, SURF_PARTS );
	}
}
